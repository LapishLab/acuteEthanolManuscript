...
interpolateEnPred;
itpRnnPred = itpRnnPred';
newColumn = [16, 17, 18, 19, 37, 38];
numColumns = size(itpRnnPred, 2) + 1;
itpRnnPred(:, numColumns) = newColumn;
animalNumRnnEnPred = sortrows(itpRnnPred, size(itpRnnPred,2));

interpolatePred;
itpRnnPred = itpRnnPred';
newColumn = [16, 18, 19, 37, 38, 17];
numColumns = size(itpRnnPred, 2) + 1;
itpRnnPred(:, numColumns) = newColumn;
animalNumRnnEtPred = sortrows(itpRnnPred, size(itpRnnPred,2));
%% Correlate each neuron with each animals intake
% For ethanol
numNeurons = size(animalNumSpikesEt, 1);
numAnimals = size(animalNumRnnEtPred, 1);
...
for iNeuron = 1:numNeurons
    ...
    for iAnimal = 1:numAnimals
        ...
        predData = animalNumRnnEtPred(animalNumRnnEtPred(:, 181) == animalID, 1:180);
        
        % Check if the number of rows matches
        if size(neuronData, 2) == size(predData, 2)
            % Calculate correlation coefficient and p-value
            [corrCoeff, pValue] = corrcoef(neuronData, predData);
...
numAnimals = size(animalNumRnnEnPred, 1);
...
for iNeuron = 1:numNeurons
    ...
    for iAnimal = 1:numAnimals
        ...
        % Find the corresponding intake data for the current animal
        predData = animalNumRnnEnPred(animalNumRnnEnPred(:, 181) == animalID, 1:180);
